...
struct position {
        ...
        bool operator==(const position& other) const {
			...
			if ((*arrays)[i]->type_id() == arrow::Type::STRING) {
				auto left = std::static_pointer_cast<arrow::StringArray>((*arrays)[i]);
				auto right = std::static_pointer_cast<arrow::StringArray>((*other.arrays)[i]);
				if  (left->GetString(row_index) != right->GetString(other.row_index)) {
					return false;
				}
                        } else if ((*arrays)[i]->type_id() == arrow::Type::INT64) {
                                auto left = std::static_pointer_cast<arrow::Int64Array>((*arrays)[i]);
                                auto right = std::static_pointer_cast<arrow::Int64Array>((*other.arrays)[i]);
                                if  (left->Value(row_index) != right->Value(other.row_index)) {
                                        return false;
                                }
			} else {
                                auto left = std::static_pointer_cast<arrow::DoubleArray>((*arrays)[i]);
                                auto right = std::static_pointer_cast<arrow::DoubleArray>((*other.arrays)[i]);
                                if  (left->Value(row_index) != right->Value(other.row_index)) {
                                        return false;
                                }
                        }
                ...
                // Compute individual hash values for two data members and combine them using XOR and bit shifting
                        ...
                        for (int i = 0; i < (*p.arrays).size(); i++) { // TODO predefine type somehow
                                if ((*p.arrays)[i]->type_id() == arrow::Type::STRING) {
                                        ...
                                        auto array = std::static_pointer_cast<arrow::Int64Array>((*p.arrays)[i]); // TODO more
                                        answer ^= hash<int64_t>()(array->Value(p.row_index));
                                        //answer ^= ((hash<float>()(k.getM()) ^ (hash<float>()(k.getC()) << 1)) >> 1);
                                }
                        }
                        return answer;
                }
...
void group_by_sequential(std::vector<std::shared_ptr<arrow::Array>> *arrays, partial_mult_group* pg, int n) {
	//printf("%p %d %d\n", arrays, (*arrays)[0]->length(), (*arrays)[1]->length());
